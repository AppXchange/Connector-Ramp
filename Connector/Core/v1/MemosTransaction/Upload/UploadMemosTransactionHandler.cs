...
namespace Connector.Core.v1.MemosTransaction.Upload;

public class UploadMemosTransactionHandler : IActionHandler<UploadMemosTransactionAction>
{
    private readonly ILogger<UploadMemosTransactionHandler> _logger;

    public UploadMemosTransactionHandler(
        ILogger<UploadMemosTransactionHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<UploadMemosTransactionActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<UploadMemosTransactionActionOutput>();
            // response = await _apiClient.PostMemosTransactionDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetMemosTransactionDataObject(response.Data.id, cancellationToken);

            // var resource = new UploadMemosTransactionActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(MemosTransactionDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "UploadMemosTransactionHandler" };
