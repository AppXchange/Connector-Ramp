...
namespace Connector.Core.v1.VirtualCard.Create;

public class CreateVirtualCardHandler : IActionHandler<CreateVirtualCardAction>
{
    private readonly ILogger<CreateVirtualCardHandler> _logger;

    public CreateVirtualCardHandler(
        ILogger<CreateVirtualCardHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateVirtualCardActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateVirtualCardActionOutput>();
            // response = await _apiClient.PostVirtualCardDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetVirtualCardDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateVirtualCardActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(VirtualCardDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateVirtualCardHandler" };
