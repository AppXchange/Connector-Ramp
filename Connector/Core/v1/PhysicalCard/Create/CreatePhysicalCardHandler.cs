...
namespace Connector.Core.v1.PhysicalCard.Create;

public class CreatePhysicalCardHandler : IActionHandler<CreatePhysicalCardAction>
{
    private readonly ILogger<CreatePhysicalCardHandler> _logger;

    public CreatePhysicalCardHandler(
        ILogger<CreatePhysicalCardHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreatePhysicalCardActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreatePhysicalCardActionOutput>();
            // response = await _apiClient.PostPhysicalCardDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetPhysicalCardDataObject(response.Data.id, cancellationToken);

            // var resource = new CreatePhysicalCardActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(PhysicalCardDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreatePhysicalCardHandler" };
