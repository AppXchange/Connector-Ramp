...
namespace Connector.Core.v1.SharedLimit.RemoveUsers;

public class RemoveUsersSharedLimitHandler : IActionHandler<RemoveUsersSharedLimitAction>
{
    private readonly ILogger<RemoveUsersSharedLimitHandler> _logger;

    public RemoveUsersSharedLimitHandler(
        ILogger<RemoveUsersSharedLimitHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<RemoveUsersSharedLimitActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<RemoveUsersSharedLimitActionOutput>();
            // response = await _apiClient.PostSharedLimitDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetSharedLimitDataObject(response.Data.id, cancellationToken);

            // var resource = new RemoveUsersSharedLimitActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(SharedLimitDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "RemoveUsersSharedLimitHandler" };
