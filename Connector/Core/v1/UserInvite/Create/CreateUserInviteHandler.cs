...
namespace Connector.Core.v1.UserInvite.Create;

public class CreateUserInviteHandler : IActionHandler<CreateUserInviteAction>
{
    private readonly ILogger<CreateUserInviteHandler> _logger;

    public CreateUserInviteHandler(
        ILogger<CreateUserInviteHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateUserInviteActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateUserInviteActionOutput>();
            // response = await _apiClient.PostUserInviteDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetUserInviteDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateUserInviteActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(UserInviteDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateUserInviteHandler" };
