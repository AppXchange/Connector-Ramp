...
namespace Connector.Core.v1.Location.Create;

public class CreateLocationHandler : IActionHandler<CreateLocationAction>
{
    ...
    {
        var input = JsonSerializer.Deserialize<CreateLocationActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateLocationActionOutput>();
            // response = await _apiClient.PostLocationDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetLocationDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateLocationActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(LocationDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateLocationHandler" };
