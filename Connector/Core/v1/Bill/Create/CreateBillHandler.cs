...
namespace Connector.Core.v1.Bill.Create;

public class CreateBillHandler : IActionHandler<CreateBillAction>
{
    ...
    {
        var input = JsonSerializer.Deserialize<CreateBillActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateBillActionOutput>();
            // response = await _apiClient.PostBillDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetBillDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateBillActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(BillDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateBillHandler" };
