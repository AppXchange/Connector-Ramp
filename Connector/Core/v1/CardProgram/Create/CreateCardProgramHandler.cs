...
public class CreateCardProgramHandler : IActionHandler<CreateCardProgramAction>
{
    private readonly ILogger<CreateCardProgramHandler> _logger;

    public CreateCardProgramHandler(
        ILogger<CreateCardProgramHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateCardProgramActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateCardProgramActionOutput>();
            // response = await _apiClient.PostCardProgramDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetCardProgramDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateCardProgramActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(CardProgramDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateCardProgramHandler" };
