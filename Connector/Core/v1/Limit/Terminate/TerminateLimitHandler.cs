...
namespace Connector.Core.v1.Limit.Terminate;

public class TerminateLimitHandler : IActionHandler<TerminateLimitAction>
{
    private readonly ILogger<TerminateLimitHandler> _logger;

    public TerminateLimitHandler(
        ILogger<TerminateLimitHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<TerminateLimitActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<TerminateLimitActionOutput>();
            // response = await _apiClient.PostLimitDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetLimitDataObject(response.Data.id, cancellationToken);

            // var resource = new TerminateLimitActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(LimitDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "TerminateLimitHandler" };
