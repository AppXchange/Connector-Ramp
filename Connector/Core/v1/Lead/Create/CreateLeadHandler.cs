...
namespace Connector.Core.v1.Lead.Create;

public class CreateLeadHandler : IActionHandler<CreateLeadAction>
{
    ...
    {
        var input = JsonSerializer.Deserialize<CreateLeadActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateLeadActionOutput>();
            // response = await _apiClient.PostLeadDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetLeadDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateLeadActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(LeadDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateLeadHandler" };
