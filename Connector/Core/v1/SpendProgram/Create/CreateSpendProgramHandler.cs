...
namespace Connector.Core.v1.SpendProgram.Create;

public class CreateSpendProgramHandler : IActionHandler<CreateSpendProgramAction>
{
    private readonly ILogger<CreateSpendProgramHandler> _logger;

    public CreateSpendProgramHandler(
        ILogger<CreateSpendProgramHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateSpendProgramActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateSpendProgramActionOutput>();
            // response = await _apiClient.PostSpendProgramDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetSpendProgramDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateSpendProgramActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(SpendProgramDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateSpendProgramHandler" };
