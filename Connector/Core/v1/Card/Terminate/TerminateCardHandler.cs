...
namespace Connector.Core.v1.Card.Terminate;

public class TerminateCardHandler : IActionHandler<TerminateCardAction>
{
    private readonly ILogger<TerminateCardHandler> _logger;

    public TerminateCardHandler(
        ILogger<TerminateCardHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<TerminateCardActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<TerminateCardActionOutput>();
            // response = await _apiClient.PostCardDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetCardDataObject(response.Data.id, cancellationToken);

            // var resource = new TerminateCardActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(CardDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "TerminateCardHandler" };
