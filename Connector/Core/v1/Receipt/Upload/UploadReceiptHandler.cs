...
namespace Connector.Core.v1.Receipt.Upload;

public class UploadReceiptHandler : IActionHandler<UploadReceiptAction>
{
    private readonly ILogger<UploadReceiptHandler> _logger;
    ...
    {
        var input = JsonSerializer.Deserialize<UploadReceiptActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<UploadReceiptActionOutput>();
            // response = await _apiClient.PostReceiptDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetReceiptDataObject(response.Data.id, cancellationToken);

            // var resource = new UploadReceiptActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(ReceiptDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "UploadReceiptHandler" };
