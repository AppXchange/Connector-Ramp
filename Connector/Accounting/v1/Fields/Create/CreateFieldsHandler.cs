...
namespace Connector.Accounting.v1.Fields.Create;

public class CreateFieldsHandler : IActionHandler<CreateFieldsAction>
{
    private readonly ILogger<CreateFieldsHandler> _logger;

    public CreateFieldsHandler(
        ILogger<CreateFieldsHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateFieldsActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateFieldsActionOutput>();
            // response = await _apiClient.PostFieldsDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetFieldsDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateFieldsActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(FieldsDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateFieldsHandler" };
