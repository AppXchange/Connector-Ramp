...
namespace Connector.Accounting.v1.Sync.PostStatus;

public class PostStatusSyncHandler : IActionHandler<PostStatusSyncAction>
{
    private readonly ILogger<PostStatusSyncHandler> _logger;

    public PostStatusSyncHandler(
        ILogger<PostStatusSyncHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<PostStatusSyncActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<PostStatusSyncActionOutput>();
            // response = await _apiClient.PostSyncDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetSyncDataObject(response.Data.id, cancellationToken);

            // var resource = new PostStatusSyncActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(SyncDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "PostStatusSyncHandler" };
