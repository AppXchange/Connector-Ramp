...
namespace Connector.Accounting.v1.Accounts.Upload;

public class UploadAccountsHandler : IActionHandler<UploadAccountsAction>
{
    private readonly ILogger<UploadAccountsHandler> _logger;

    public UploadAccountsHandler(
        ILogger<UploadAccountsHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<UploadAccountsActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<UploadAccountsActionOutput>();
            // response = await _apiClient.PostAccountsDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetAccountsDataObject(response.Data.id, cancellationToken);

            // var resource = new UploadAccountsActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(AccountsDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "UploadAccountsHandler" };
