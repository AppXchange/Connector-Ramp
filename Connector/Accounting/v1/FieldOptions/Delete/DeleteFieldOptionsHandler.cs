...
namespace Connector.Accounting.v1.FieldOptions.Delete;

public class DeleteFieldOptionsHandler : IActionHandler<DeleteFieldOptionsAction>
{
    private readonly ILogger<DeleteFieldOptionsHandler> _logger;

    public DeleteFieldOptionsHandler(
        ILogger<DeleteFieldOptionsHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<DeleteFieldOptionsActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<DeleteFieldOptionsActionOutput>();
            // response = await _apiClient.PostFieldOptionsDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetFieldOptionsDataObject(response.Data.id, cancellationToken);

            // var resource = new DeleteFieldOptionsActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(FieldOptionsDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "DeleteFieldOptionsHandler" };
