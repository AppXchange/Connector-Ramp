...
namespace Connector.Accounting.v1.Connection.Delete;

public class DeleteConnectionHandler : IActionHandler<DeleteConnectionAction>
{
    private readonly ILogger<DeleteConnectionHandler> _logger;

    public DeleteConnectionHandler(
        ILogger<DeleteConnectionHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<DeleteConnectionActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<DeleteConnectionActionOutput>();
            // response = await _apiClient.PostConnectionDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetConnectionDataObject(response.Data.id, cancellationToken);

            // var resource = new DeleteConnectionActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(ConnectionDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "DeleteConnectionHandler" };
