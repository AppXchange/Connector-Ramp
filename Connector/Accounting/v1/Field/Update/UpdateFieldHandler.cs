...
namespace Connector.Accounting.v1.Field.Update;

public class UpdateFieldHandler : IActionHandler<UpdateFieldAction>
{
    ...
    {
        var input = JsonSerializer.Deserialize<UpdateFieldActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<UpdateFieldActionOutput>();
            // response = await _apiClient.PostFieldDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetFieldDataObject(response.Data.id, cancellationToken);

            // var resource = new UpdateFieldActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(FieldDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "UpdateFieldHandler" };
