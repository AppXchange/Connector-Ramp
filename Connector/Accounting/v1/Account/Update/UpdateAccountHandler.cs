...
namespace Connector.Accounting.v1.Account.Update;

public class UpdateAccountHandler : IActionHandler<UpdateAccountAction>
{
    ...
    {
        var input = JsonSerializer.Deserialize<UpdateAccountActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<UpdateAccountActionOutput>();
            // response = await _apiClient.PostAccountDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetAccountDataObject(response.Data.id, cancellationToken);

            // var resource = new UpdateAccountActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(AccountDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "UpdateAccountHandler" };
